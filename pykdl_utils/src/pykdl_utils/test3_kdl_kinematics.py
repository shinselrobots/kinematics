...
def do_right_arm():
    ...
    if q_new is not None:
        ...
        print ""    
        print "This shows that the IK is not suitable for Sheldon!"            
        print "Cant easily set knee and hip to not changable"             
        print "======== COMPARISON =============" 
        print " out:          in:          Difference:"
        for idx, val in enumerate(joint_angles):
            print '% 1.6f'% (q_new[idx]), " - ", \
                '% 1.6f'% (joint_angles[idx]), " = " '% 1.6f'% (q_new[idx] - joint_angles[idx])
        print "================================="  
